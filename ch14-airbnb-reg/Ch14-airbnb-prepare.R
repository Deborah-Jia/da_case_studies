...
# data_dir must be first defined #
# data_dir <- '/Users/wodediannao/Desktop/da_data_repo' # must be absolute path
...
data_in <- paste(data_dir,"airbnb","clean/", sep = "/") # path for saving clean data in data folder

use_case_dir <- "ch14-airbnb-reg/"
data_out <- use_case_dir # path for case studies
output <- paste0(use_case_dir,"output/") # path for case studies (extensive folders)
...
data <- data %>%
  mutate( # if "==", then change name, or else keep it original
    ...
    f_property_type = factor(property_type)) # why create a duplicate variable
...
numericals <- c("accommodates","bathrooms","review_scores_rating","number_of_reviews",
                "guests_included", "reviews_per_month","extra_people","minimum_nights","beds")
...
# Pool accommodations with 0,1,2,10 bathrooms # round the number accoring to c()
...
data <- data %>%
  mutate(f_number_of_reviews = cut(n_number_of_reviews, c(0,1,51,max(data$n_number_of_reviews)), 
                                   labels=c(0,1,2), right = F))
data %>%
